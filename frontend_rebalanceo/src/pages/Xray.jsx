...
import { Loader2, Search, Globe2, Coins, PieChart, Layers, Building2 } from 'lucide-react';
...
const BarList = ({ items, tone = 'country', pretty = (x) => x, max = 8 }) => {
...
export const Xray = ({ portfolios, activePortfolioId }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }} className="space-y-8">
            {/* Controls */}
            <GlassCard className="!p-4 flex flex-col md:flex-row items-stretch md:items-center gap-3 sticky top-4 z-40">
                ...
                <div className="md:ml-auto w-full md:w-72">
                    <Dropdown value={filter} onChange={setFilter} options={etfOptions} icon={Layers} align="right" />
                </div>
            </GlassCard>
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                        <GlassCard>
                            ...
                            <div className="text-2xl font-black text-slate-800 dark:text-slate-100">{formatNumber(xray.total)} €</div>
                        </GlassCard>
                        <GlassCard>
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('xray.holdings')}</div>
                            <div className="text-2xl font-black text-slate-800 dark:text-slate-100">{realHoldings}</div>
                        </GlassCard>
                        <GlassCard>
                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('xray.top_country')}</div>
                            <div className="text-lg font-black text-slate-800 dark:text-slate-100 truncate">{topCountry ? topCountry.name : '—'}</div>
                        ...
                        <GlassCard>
                            ...
                            <div className="text-lg font-black text-slate-800 dark:text-slate-100 truncate">{topSector ? prettySector(topSector.name) : '—'}</div>
                            ...
                            <GlassCard className="!p-0 overflow-hidden">
                                <div className="p-5 border-b border-slate-100 dark:border-slate-800">
                                    ...
                                    <div className="flex items-center bg-slate-50 dark:bg-slate-800 rounded-xl px-3 py-2 border border-slate-100 dark:border-slate-700 focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all">
                                        ...
                                        <input value={query} onChange={e => setQuery(e.target.value)} placeholder={t('xray.search')} className="bg-transparent w-full outline-none text-sm font-bold text-slate-700 dark:text-slate-200 placeholder:text-slate-400" />
                                    </div>
                                </div>
                                <div className="max-h-[520px] overflow-y-auto custom-scrollbar divide-y divide-slate-50 dark:divide-slate-800">
                                    {companies.length === 0 ? (
                                        <div className="p-8 text-center text-xs font-bold text-slate-400">{t('xray.no_match')}</div>
                                    ) : companies.map((c, idx) => (
                                        <div key={c.key} className="p-4 px-5 flex items-center gap-4 hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                                            <div className="w-6 text-center text-[11px] font-black text-slate-300 dark:text-slate-600 shrink-0">{idx + 1}</div>
                                            <div className="min-w-0 flex-1">
                                                <div className={`text-sm font-bold truncate ${c.other ? 'text-slate-400 italic' : 'text-slate-800 dark:text-slate-100'}`}>{c.name}</div>
                                                <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                                                    {c.symbol && !c.other && <span className="text-[10px] font-black text-indigo-400">{c.symbol}</span>}
                                                    {!c.other && c.sources.map(s => (
                                                        <span key={s} className="text-[9px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{s === c.symbol ? t('xray.direct') : s}</span>
                                                    ))}
                                                </div>
                                            </div>
                                            <div className="w-28 shrink-0">
                                                ...
                                                <div className="text-right text-[10px] font-black text-slate-400 tabular-nums">{formatNumber(c.pct, 2)}%</div>
                                            </div>
                                            <div className="w-20 text-right text-sm font-black text-slate-700 dark:text-slate-200 shrink-0 tabular-nums">{formatNumber(c.value)}€</div>
                                        </div>
                                    ))}
                                </div>
                            </GlassCard>
