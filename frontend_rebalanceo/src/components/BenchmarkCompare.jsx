    ...
    return (
        <span className="relative inline-block align-baseline" ref={ref}>
            <button
                ...
                aria-expanded={open}
                onClick={() => setOpen(o => !o)}
                className="inline-flex items-center gap-1 font-black text-slate-600 dark:text-slate-200 border-b border-dashed border-slate-300 dark:border-slate-600 hover:text-indigo-600 dark:hover:text-indigo-300 hover:border-indigo-400 focus:outline-none focus-visible:text-indigo-600 transition-colors"
            >
...
const MetricTile = ({ title, state, valueCls, note, warn, t }) => (
    <div className="flex-1 min-w-[150px] rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 px-4 py-3">
        <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{title}</span>
            {warn && <Info size={12} className="text-amber-500 shrink-0" />}
        </div>
        {state.kind === 'value' ? (
            <p className={`mt-1 text-2xl font-black tabular-nums ${valueCls}`}>{formatNumber(state.value, 2)}</p>
        ) : (
            <p className="mt-1.5 text-sm font-black text-slate-400 dark:text-slate-500">
                {t(state.kind === 'error' ? 'bench.rel_not_computable' : 'bench.rel_insufficient')}
            </p>
        )}
        <p className={`mt-1 text-[11px] font-bold leading-snug ${warn ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400'}`}>{note}</p>
    </div>
);
...
export const BenchmarkCompare = ({ holdings, period }) => {
    ...
    if (hasHoldings && selected.length === 0) {
        relativeSection = (
            <div className="mt-5 rounded-[1.5rem] border border-dashed border-slate-200 dark:border-slate-700 px-5 py-6 text-center">
                <p className="text-xs font-bold text-slate-400">{t('bench.rel_empty')}</p>
            </div>
        );
    } else if (hasHoldings && !loading && data && activeRef) {
        const unavailable = beta.kind !== 'value' && corr.kind !== 'value';
        relativeSection = (
            <div className="mt-5 rounded-[1.5rem] bg-slate-50/70 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 p-5">
                <div className="flex items-center gap-2">
                    <Activity size={15} className="text-slate-400" />
                    <h4 className="text-xs font-black text-slate-500 dark:text-slate-300 uppercase tracking-widest">{t('bench.rel_title')}</h4>
                </div>

                {/* <div>, not <p>: the picker's popup is a block element. */}
                <div className="mt-1.5 mb-4 text-[11px] font-medium text-slate-400">
                    {t('bench.rel_ref')}:{' '}
                    {selected.length > 1 ? (
                        <InlineSelect
                            value={activeRef}
                        ...
                        <span className="font-black text-slate-600 dark:text-slate-200">{labelFor(activeRef)}</span>
                    )}
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                    <MetricTile
                        ...
                        valueCls={beta.kind === 'value' && beta.value < 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-700 dark:text-slate-100'}
                    ...
                    <MetricTile
                        ...
                        valueCls={corr.kind === 'value' && corr.value < 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-700 dark:text-slate-100'}
                ...
                {!unavailable && rel?.points > 0 && (
                    <p className="mt-3 text-[10px] font-bold text-slate-400">{formatNumber(rel.points)} {t('bench.rel_points')}</p>
                )}
            </div>
        );
    }
                                        ...
                                        <tr key={key} className={isPort ? 'bg-indigo-50/40 dark:bg-indigo-900/10' : ''}>
                                            <td className="py-3 pr-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-xs font-black ${isPort ? 'text-indigo-600 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-200'}`}>{isPort ? t('bench.your_portfolio') : labelFor(key)}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-3 pl-2 text-right text-xs font-bold tabular-nums text-rose-500 dark:text-rose-400">{formatNumber(s.max_drawdown, 1)}%</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
