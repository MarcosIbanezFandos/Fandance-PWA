                                ...
                                <tbody className="divide-y divide-slate-50">
                                    <AnimatePresence>
                                        {portfolioItems.map(item => (
                                            <motion.tr key={item.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, height: 0 }} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                                <td className="p-5 pl-8"><div className="font-bold text-xs text-slate-800 dark:text-slate-100">{item.asset?.name || item.asset?.ticker}</div><div className="text-[10px] font-bold text-indigo-400">{item.asset?.ticker}</div></td>
                                                <td className="p-5 text-right text-xs font-mono text-slate-500 dark:text-slate-400">{item.current_price?.toFixed(2)}€</td>
                                                <td className="p-5 text-center"><input className="w-16 bg-slate-100 dark:bg-slate-800 dark:text-slate-100 rounded-lg p-2 text-center text-xs font-bold focus:bg-white dark:focus:bg-slate-700 focus:ring-2 ring-indigo-500 outline-none transition-all" value={item.units_held} onChange={e => handleUpdate(item.id, 'units_held', e.target.value)} onFocus={e => e.target.select()} /></td>
                                                <td className="p-5 text-center"><div className="relative inline-block"><input className="w-24 bg-white dark:bg-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-center text-xs font-bold focus:ring-2 ring-emerald-500 outline-none transition-all" value={item.value?.toFixed(2)} onChange={e => handleUpdate(item.id, 'value', e.target.value)} onFocus={e => e.target.select()} /><span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 font-bold">€</span></div></td>
                                                <td className="p-5 text-center"><input className="w-12 bg-slate-100 dark:bg-slate-800 dark:text-slate-100 rounded-lg p-2 text-center text-xs font-bold focus:bg-white dark:focus:bg-slate-700 focus:ring-2 ring-indigo-500 outline-none transition-all" value={item.target_weight} onChange={e => handleUpdate(item.id, 'target_weight', e.target.value)} onFocus={e => e.target.select()} /></td>
                                                <td className="p-5 text-center bg-indigo-50/10 dark:bg-indigo-900/20"><div className={`inline-flex items-center px-2 py-1 rounded-md text-[10px] font-black uppercase ${item.action === 'BUY' ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30' : 'text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/30'}`}>{item.action === 'BUY' ? <ArrowUpRight size={10} className="mr-1" /> : <ArrowDownRight size={10} className="mr-1" />}{Math.abs(item.unitsToTrade || 0).toFixed(3)}</div></td>
                                                <td className={`p-5 text-right text-xs font-bold font-mono bg-indigo-50/10 dark:bg-indigo-900/20 ${item.action === 'BUY' ? 'text-slate-700 dark:text-slate-300' : 'text-rose-500 dark:text-rose-400'}`}>{Math.abs(item.diffVal || 0).toLocaleString('es-ES', { maximumFractionDigits: 0 })}€</td>
                                                <td className="p-5 text-right"><button onClick={() => deleteItem(item.id)} className="text-slate-300 dark:text-slate-500 hover:text-rose-500 transition-colors"><Trash2 size={16} /></button></td>
                                            </motion.tr>
                                        ))}
                                    </AnimatePresence>
                                </tbody>
