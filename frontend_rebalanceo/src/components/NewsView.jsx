            ...
            {newsData.aggregate && (
                <GlassCard className="flex flex-col md:flex-row items-center justify-between gap-6 !p-8 bg-gradient-to-r from-white to-slate-50">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="text-sm font-black text-slate-400 uppercase tracking-widest">Global Portfolio Sentiment (RSI)</div>
                            ...
                            <div key={ticker} className="space-y-6">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black text-sm shadow-lg">
                                    ...
                                    {sentiment && (
                                        <div className={`flex items-center gap-3 px-4 py-2 rounded-xl border ${getBadge(sentiment.color).split(' ')[0].replace('bg-', 'border-').replace('100', '200')} ${getBadge(sentiment.color)}`}>
                                            <div className="text-right">
                                                <div className="text-[10px] font-black uppercase tracking-widest opacity-80">RSI Indicator</div>
                                    ...
                                    {items.map((item, idx) => (
                                        <GlassCard key={idx} className="flex flex-col hover:-translate-y-1 transition-transform duration-300 !p-0 overflow-hidden group h-full">
                                            ...
                                            <div className="p-5 flex flex-col flex-1">
                                                ...
                                                <h3 className="text-sm font-bold text-slate-800 leading-snug mb-4 line-clamp-3 group-hover:text-indigo-600 transition-colors">
                                                    {item.title}
                                                </h3>
                                                ...
                                                <div className="mt-auto pt-4 border-t border-slate-50">
                                                    <a
                                                        ...
                                                        className="flex items-center justify-between text-[10px] font-black text-indigo-500 hover:text-indigo-700 transition-colors uppercase tracking-widest"
                                                    >
